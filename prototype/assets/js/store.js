...
export function login(u){for(const key of Object.keys(sessionStorage)){if(key.startsWith('pcmatch-form:')&&key.endsWith(':guest')){sessionStorage.setItem(key.slice(0,-5)+u.id,sessionStorage.getItem(key));sessionStorage.removeItem(key);}}sessionStorage.setItem('pcmatch-session',JSON.stringify({userId:u.id,expires:Date.now()+3600000}));}
export function logout(){sessionStorage.removeItem('pcmatch-session');}
