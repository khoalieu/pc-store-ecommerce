...
import {homeBanners,latestModel,bySold} from './merchandising-data.js';
...
function newest(m){if(!m)return '';const c=commercial(m);return '<div class="home-newest-copy"><h2 id="newest-title">Sản phẩm mới nhất</h2><p class="muted">Theo ngày thêm vào catalogue demo · <time datetime="'+esc(m.createdAt)+'">'+new Intl.DateTimeFormat('vi-VN',{timeZone:'UTC'}).format(new Date(m.createdAt))+'</time></p><p>'+esc(categories[m.category])+'</p><h3>'+esc(m.name)+'</h3><div class="home-specs">'+specs(m)+'</div><p class="home-price">'+(c.count?'Giá demo từ <strong>'+c.price+'</strong>':c.price)+' · '+c.count+' shop còn hàng</p><div class="cluster">'+a('Xem sản phẩm','model',{id:m.id},'primary')+a('Xem các shop bán','model',{id:m.id,compareOffers:1})+'</div></div>'+image(m);}
...
export function home(){
 const latest=latestModel(models),best=models.filter(m=>Number.isFinite(m.soldCount)&&m.soldCount>=0).slice().sort(bySold).slice(0,4);
 ...
 document.body.classList.add('home-page');document.querySelector('.home-newest').hidden=!latest;
 if(latest)document.querySelector('.home-newest').dataset.modelId=latest.id;
}
